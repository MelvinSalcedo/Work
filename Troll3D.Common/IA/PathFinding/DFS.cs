...
namespace Troll3D.Common.IA.PathFinding
{
    ...
    /// ouverte, on ajoute tout ses voisins au début de la liste ouverte, on met le parent dans la liste fermé
        ...
        private void ManageNeighbours( Node node )
        {
            for ( int i = 0; i < node.NeighbourCount; i++ )
            {
                if ( IsNodeListed( node.GetNeighbour( i ) ) == false )
                {
                    ...
                    m_openList.Insert(0, fetchednode );
                }
            }
        }

        /// <summary> 
        /// Retourne la liste des noeuds traversé formant le chemin entre les noeuds
        /// de début et d'arrivé
        /// </summary>
